...
function Navbar() {
  ...
  return (
    ...
    <div className='flex justify-between h-[60px] items-center bg-white'>
      ...
      <NavLink to='/profile' className={({ isActive }) => `font-medium flex gap-4 mr-12 items-center ${getActiveClass({ isActive })}`}>
        <p>Welcome, {profiledata?.name}!</p>
        <img className='h-[45px] w-[45px] rounded-full' src={profiledata?.img}></img>
      </NavLink>
    </div>
