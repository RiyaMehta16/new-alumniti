...
const Message = ({ userId }) => {
    ...
    const fetchMessages = async () => {
        ...
        try {
            const response = await axios.get(`http://localhost:5000/api/auth/messages/${userId}`, {
                headers: {
    ...
    const handleSendMessage = async () => {
        ...
        try {
            const response = await axios.post(`http://localhost:5000/api/auth/send/${userId}`, 
                ...
                <div className="message-display">
                    {messages.length === 0 ? (
                        ...
                        messages.map((message) => (
                            <div key={message._id} className="message"> {/* Using message._id as key */}
                                <p>{message.content}</p>
