...
function HeroCollabration() {
  ...
  return (
    ...
    <div className='flex justify-between items-center w-full'>
        <div className='font-medium text-xl ml-[30px]'>Recommended Collabrations</div>
        <div className='flex mr-[30px] gap-4'>
        <button onClick={()=>navigate('/createcollab')} className='bg-black text-white rounded px-6 py-2 text-sm font-semibold shadow-lg hover:bg-[#3f0071] transition-transform transform hover:scale-105'>
    ...
    {collab.map((collab, index)=>(
      <div className="bg-white shadow-lg w-[400px] rounded-md p-6 text-gray-800 border border-gray-200">
        ...
        <div className="mb-4">
           <h3 className="text-2xl font-bold ">{collab.collabTitle}</h3>
        </div>
        <div className="mb-4">
           <h4 className="text-lg font-medium text-gray-700">Roles Needed:</h4>
           <ul className="ml-4 list-disc list-inside text-gray-600">
            <li>{collab.RoleNeed}</li>
           </ul>
        </div>
        <div className="mb-4">
           <h4 className="text-lg font-medium text-gray-700">Skills Required:</h4>
           <p className="text-gray-600 ml-4">{collab.skills}</p>
        </div>
        <div className="mb-4">
           <h4 className="text-lg font-medium text-gray-700">Timeline:</h4>
           <p className="ml-4 text-gray-600">Start Date: {formatDate(collab.startDate)}</p>
           <p className="ml-4 text-gray-600">End Date: {formatDate(collab.startDate)}</p>
        </div>
        <div className="mt-6">
